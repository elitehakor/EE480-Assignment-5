...
always @(reset) begin
  ...
  // Initialize 32-bit instruction memory with two
  // words from the 16-bit memory
...
always @(posedge clk) begin
  ...
  if( oo_execuction == 2 ) begin
    // If we're here, the last instruction was the first in a
    // pair of out-of-order instructions. This is the second.
    oo_execuction <= 2'h1;
    ...
    $display("------------AFTER NEXT PAIR OF WORDS IN PIPE, QUAD OUT OF ORDER--------");
  end else begin
    // Normal execution. Toggle between high and low instructions
    oo_execuction      <= 2'h0;
    parallel_store_en  <= 1'b0;
    if (jump_taken) pc <= jump_addr;
